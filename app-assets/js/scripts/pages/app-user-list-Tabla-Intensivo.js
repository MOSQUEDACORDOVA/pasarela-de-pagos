...
$(function () {
  ...
  var dtUserTable = $('.user-list-table-Tabla-Intensivo'),
    ...
    dtUserTable.DataTable({
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var label = $('<label class="form-label" for="UserRole">Niveles</label>').appendTo('.niveles-grupos-intensivo');
            var select = $(
              '<select id="UserRole" class="form-select text-capitalize mb-md-0 mb-2"><option value=""> Todos </option></select>'
            )
              .appendTo('.niveles-grupos-intensivo')
              .on('change', function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var label = $('<label class="form-label" for="UserPlan">Profesor</label>').appendTo('.profesor-grupos-intensivo');
            var select = $(
              '<select id="UserPlan" class="form-select text-capitalize mb-md-0 mb-2"><option value=""> Todos </option></select>'
            )
              .appendTo('.profesor-grupos-intensivo')
              .on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                column.search(val ? '^' + val + '$' : '', true, false).draw();
              });
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var label = $('<label class="form-label" for="FilterTransaction">Fecha de pago</label>').appendTo('.pagos-grupos-intensivo');
            var select = $(
              '<select id="UserPlan" class="form-select text-capitalize mb-md-0 mb-2xx"><option value="">  </option></select>'
            )
              .appendTo('.pagos-grupos-intensivo')
              .on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                column.search(val ? '^' + val + '$' : '', true, false).draw();
              });
